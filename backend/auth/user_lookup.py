...
def get_user_by_id(database_path: str, user_id: str) -> dict | None:
    ...
    try:
        cur = conn.execute(
            """SELECT id, username, role, full_name, email, must_change_password, password_changed_at
...
def get_user_by_username(database_path: str, username: str) -> dict | None:
    ...
    try:
        cur = conn.execute(
            """SELECT id, username, role, password_hash, full_name, email, must_change_password, password_changed_at
