...
def get_user_by_id(database_path: str, user_id: str) -> dict | None:
    """Return user row as dict (id, username, role) or None."""
    conn = get_connection(database_path)
    try:
        cur = conn.execute(
            "SELECT id, username, role FROM users WHERE id = ? AND disabled_at IS NULL",
            (user_id,),
        )
        row = cur.fetchone()
        if not row:
...
def get_user_by_username(database_path: str, username: str) -> dict | None:
    """Return user row as dict (id, username, role, password_hash) or None."""
    conn = get_connection(database_path)
    try:
        cur = conn.execute(
            "SELECT id, username, role, password_hash FROM users WHERE username = ? AND disabled_at IS NULL",
            (username,),
        )
        row = cur.fetchone()
        if not row:
